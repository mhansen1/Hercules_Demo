...
void ESM_ApplicationCallback(uint32 grp_channel, uint32 param1, uint32 param2, uint32 param3)
{
...
#if defined(_TMS570LS31x_) || defined(_TMS570LS12x_) || defined(_RM48x_) || defined(_RM46x_) || defined(_RM42x_) || defined(_TMS570LS04x_)
	if((1 == ((grp_channel|ESM_GRP1_MASK)>>16))&&(ESM_G1ERR_PSCON_COMPARE_ERR == (grp_channel & 0x0000ffff)))
	{
	...
	else if((1 == ((grp_channel|ESM_GRP1_MASK)>>16))&&(ESM_G1ERR_PSCON_SELTEST_ERR == (grp_channel & 0x0000ffff)))
	{
		/*clear nerror and esm flags*/
		sl_esmREG->SR4[0] |= GET_ESM_BIT_NUM(ESM_G1ERR_PSCON_SELTEST_ERR);
		_SL_HoldNClear_nError();
	...
	else if((1 == ((grp_channel|ESM_GRP1_MASK)>>16))&&(ESM_G1ERR_CCMR4_SELFTEST == (grp_channel & 0x0000ffff)))
	{
	...
	else if((1 == ((grp_channel|ESM_GRP1_MASK)>>16))&&(ESM_G1ERR_EEPROM_CORRERR == (grp_channel & 0x0000ffff)))
	{
		/* clear nError PIN and esm flags */
	...
	else if((1 == ((grp_channel|ESM_GRP1_MASK)>>16))&&(ESM_G1ERR_EEPROM_UNCORRERR == (grp_channel & 0x0000ffff)))
	{
		/* clear nError PIN and esm flags */
	...
	else if((1 == ((grp_channel|ESM_GRP1_MASK)>>16))&&(ESM_G1ERR_B0TCM_CORRERR == (grp_channel & 0x0000ffff)))
	{
		/* clear nError PIN and esm flags */
	...
	else if((1 == ((grp_channel|ESM_GRP1_MASK)>>16))&&(ESM_G1ERR_B1TCM_CORRERR == (grp_channel & 0x0000ffff)))
	{
	...
	/********************************************************************************************************/
	else if((2 == ((grp_channel|ESM_GRP1_MASK)>>16))&&(ESM_G2ERR_CCMR4_COMP == (grp_channel & 0x0000ffff)))
	{
		/* clear nError PIN and esm flags */
	    sl_esmREG->SR1[1] |= GET_ESM_BIT_NUM(ESM_G2ERR_CCMR4_COMP);
	    sl_esmREG->SSR2 |= GET_ESM_BIT_NUM(ESM_G2ERR_CCMR4_COMP);
	...
	else if((2 == ((grp_channel|ESM_GRP2_MASK)>>16))&&(ESM_G2ERR_B0TCM_UNCORR == (grp_channel & 0x0000ffff)))
	{
		/* clear nError PIN and esm flags */
	...
	else if((2 == ((grp_channel|ESM_GRP2_MASK)>>16))&&(ESM_G2ERR_B1TCM_UNCORR == (grp_channel & 0x0000ffff)))
	{
		sl_esmREG->SR1[1] |= GET_ESM_BIT_NUM(ESM_G2ERR_B1TCM_UNCORR);
		sl_esmREG->SSR2 |= GET_ESM_BIT_NUM(ESM_G2ERR_B1TCM_UNCORR);
		sram_radecode_diagnostics_app_callback = TRUE;
	}
	else if((2 == ((grp_channel|ESM_GRP2_MASK)>>16))&&(ESM_G2ERR_FMC_UNCORR == (grp_channel & 0x0000ffff)))
	{
	...
	/********************************************************************************************************/
	else if((3 == ((grp_channel|ESM_GRP3_MASK)>>16))&&(ESM_G3ERR_B0TCM_ECC_UNCORR == (grp_channel & 0x0000ffff)))
	{
		/* Clear nError */
		_SL_HoldNClear_nError();
	...
	else if((3 == ((grp_channel|ESM_GRP3_MASK)>>16))&&(ESM_G3ERR_B1TCM_ECC_UNCORR == (grp_channel & 0x0000ffff)))
	{
		/* Clear nError */
		_SL_HoldNClear_nError();
		/* Clear the ESM Status */
	...
	else if((3 == ((grp_channel|ESM_GRP3_MASK)>>16))&&(ESM_G3ERR_FMC_UNCORR == (grp_channel & 0x0000ffff)))
	{
		/* Clear nError */
